...
class ReturnController extends Controller
{
    ...
    public function EmployeeReturnRequest(){

        $orders = Order::where('return_order',1)->orderBy('id','DESC')->get();
        return view('backend.employees.8ray.return_order.return_request',compact('orders'));

    } // End Method

    public function EmployeeReReturnRequestApproved(Request $request,$order_id)
    {
        // Update the order's return status
        Order::where('id', $order_id)->update(['return_order' => 2]);
        ...
        return redirect()->route('employee.return.request')->with($notification);
    }



    public function EmployeeReCompleteReturnRequest(){

        $orders = Order::where('return_order',2)->orderBy('id','DESC')->get();
        return view('backend.employees.8ray.return_order.complete_return_request',compact('orders'));

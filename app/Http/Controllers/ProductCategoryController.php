...
class ProductCategoryController extends Controller
{
    ...
    // Start Employee Product Category

    public function AllEmployeeProductCategories(){
        $product_categories = ProductCategory::latest()->get();
        return view('backend.employees.8ray.product_category.product_category_all',compact('product_categories'));
    } // End Method

    public function StoreEmployeeProductCategories(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'product_category_name' => 'required|string|max:255|unique:product_categories,product_category_name',
            'product_category_image' => 'required|image|mimes:jpeg,png,jpg|max:2048',
        ...
        return redirect()->route('all.employee.product.categories')->with($notification);
    }

    public function EditEmployeeProductCategories($slug)
    {
        $product_category = ProductCategory::where('product_category_slug', $slug)->firstOrFail();
        $product_categories = ProductCategory::latest()->get();
        return view('backend.employees.8ray.product_category.product_category_edit', compact('product_category', 'product_categories'));
    }

    public function UpdateEmployeeProductCategories(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'product_category_name' => 'required|string|max:255|unique:product_categories,product_category_name',
            'product_category_image' => 'nullable|image|mimes:jpeg,png,jpg|max:2048',
    ...
    public function DestoryEmployeeProductCategories($id)
    {
        $product_category = ProductCategory::findOrFail($id);
        $imagePath = public_path('upload/product_category_images/' . $product_category->product_category_image);
        if (file_exists($imagePath) && is_file($imagePath)) {

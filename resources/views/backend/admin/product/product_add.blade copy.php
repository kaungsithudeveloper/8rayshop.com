    ...
    <link href="{{ url('backend/plugins/typeahead/typeaheadjs.min.css') }}" rel="stylesheet" />
            ...
            <script>
                    ...
                    function onInput(e) {
                        ...
                        fetch(`/product-colors?query=${value}`)
                            ...
                            .then(colors => {
                                ...
                                tagify.settings.whitelist.splice(0, tagify.settings.whitelist.length, ...colors);
                                tagify.dropdown.show.call(tagify, value); // render the suggestions dropdown
                            });
                    }

                    function onAdd(e) {
                        const input = e.detail.tagify.DOM.input;
                        const lastWord = input.value.split(',').pop().trim();
                        input.value = capitalizeWords(lastWord);
                    }

                    const inputElements = document.querySelectorAll('.product-color-input');
                    inputElements.forEach(input => initializeTagify(input));
                });
                ...
                $(document).on('click', '.add-color-btn', function() {
                    ...
                    $('#color-input-container').append(colorInputHTML);
                    initializeTagify(document.querySelector('.product-color-input:last-child'));
                });
